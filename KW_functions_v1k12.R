...
alpha_diversity_v2_KW <- function(input_phyloseq, 
  ...
  p <- p + theme_classic() +
    ...
    scale_fill_manual(values=sample_type2_color_t) +
    scale_x_discrete(labels=sample_types) +
    labs(title=plot_title, x = xlabel, y= ylabel) +
    theme(plot.title=element_text(hjust=0.5, face="bold"),
          axis.title=element_text(face="bold", size=axis_title_size),
          axis.text.x =element_text(size=xlabel_size),
          axis.text.y =element_text(size=ylabel_size),
...
Lefse_phylo_KW <- function(input_phyloseq, 
  ...
  resdf$Taxa2 <- factor(resdf$Taxa2, levels = unique(resdf$Taxa2[order(resdf$indexx)]))
  ...
  if (graph_option=="none") {
    ...
    dev.off() 
          ...
          rel.otu.total4_box$features <- factor(rel.otu.total4_box$features, levels = unique(rel.otu.total4_box$features[order(rel.otu.total4_box$indexx)]))
          ...
          rel.otu.total4_final$features <- factor(rel.otu.total4_final$features, levels = unique(rel.otu.total4_final$features[order(rel.otu.total4_final$indexx)]))
...
PLS_DA_nonPS_KW <- function(input_table, 
                      ...
                      top_display_way=c("top_overall", "top_each_side"),
  ...
  if(missing(top_display_way)) top_display_way<-"top_each_side"
  top_display_way <- match.arg(top_display_way)
  ...
  if(top_display_way=="top_each_side") {
            p4<<-plotLoadings(MyResult.splsda, 
                             contrib = 'max', method = 'mean',
                             comp=1, name.var=col_name) ##display top components
            p4_sorted <- p4[order(p4$importance), ]
            p4_final <- rbind(head(p4_sorted, top_display_loading), tail(p4_sorted, top_display_loading))
  }else if (top_display_way=="top_overall") {
           p4_final<-plotLoadings(MyResult.splsda, 
                             contrib = 'max', method = 'mean',
                             comp=1, ndisplay = top_display_loading, name.var=col_name) ##display top components
  }
  if(flip_loading_direction=="yes"){
    p4_final$importance<-p4_final$importance*-1
    p <- ggplot(p4_final, aes(x = reorder(rownames(p4_final), importance), y = importance))+
...
Edger_nonPS_KW <- function (input_table, 
                            ...
                            lollipop_label_bold="",lollipop_bold_color="blue",
                            label_only_volcano, 
                            output_name,
                            legend_onplot=c("yes","no"),
                            plot_title="EgdeR",
                            output_format=c("pdf", "png"),
                            width, height) {
  #new options added 2/21/24  
  #volcano_label_bold : see which label you want to bold. you put in the list of label
  #volcano_bold_size: you can change the size of the label you want to bold
  #volcano_bold_color: you can change the color of the label you want to bold
  #label_only_volcano: you put in a list of label you want to display
  #if volcano_label_bold and label_only_volcano then it will display labels you specify and all the labels are bold
  #now there will be an export dataframe with output_name that you can use to extract the label names
  ...
  #this export is needed so that even if there are no significant results, it will export a csv file
  csv_output2<-paste0(output_name,"_all.csv")
  write.csv(resNoFilt_export,file=csv_output2, row.names = TRUE)
  if (dim(checkifallzero)[1] == 0) {
    print(paste0("no Taxa with FDR more than",FDR_cut_off))
  ...
  #exporting the results into a dataframe so that we can obtain the label information 
  print(paste0(output_name,": contains the EdgeR result"))
  assign(output_name, res_all, envir = .GlobalEnv)
  ...
  if (graph_option=="volcano") {
    ...
    #////////////////////Lollipop PLOT///////////////////////
    #=========================================================
    #order the lollipop labeling so it is the same order as the label within res dataframe 
    res_reorder<-res[order(-res$start), ]
    label_color = ifelse(res_reorder$Name %in% lollipop_label_bold, lollipop_bold_color, "black")
    label_face = ifelse(res_reorder$Name %in% lollipop_label_bold, "bold", "plain")
    
    p <- ggplot(res, aes(y=reorder(Name, -start), x=logFC, fill=col, size=abundance)) +
      geom_point(color="black", alpha=0.8, shape=21) +
      geom_segment(data=res[res$adj.P.Val < 0.2,], aes(yend=reorder(Name, -start)), 
                   xend=(-30), color="black", linetype="solid", linewidth=1) + 
      ...
      theme(
        ...
        axis.text.y = element_text(face=label_face, size=10, color=label_color),  # Apply color and face here
        axis.ticks = element_line(colour="gray70"),
        legend.background = element_rect(color=NA),
        legend.key = element_rect(colour="transparent", fill="white"),
        plot.title = element_text(size=23, face="bold")
  ...
  res_export_removeresults<<-res ###added 3_21_24 to remove photosynthesis results
  ##save the legend on a separate png file              
  if (legend_onplot=="no") { #if want to hide the legend on the plot, then will save the legend on separate png
    leg <- get_legend(p)
    legend<-as_ggplot(leg)
...
heatmap_nonPS_KW <- function(input_table, 
  ...
  if(missing(need_legend))  
    need_legend<-"yes"  #default being yes for need legend, unless specified
  ...
  pdf(file=pdf_output, width=width, height=height)
          ...
          if (need_legend=="yes"){legend(legend_position, title = sample_type_var_name,legend=sample_types, fill=sample_type_color, cex=legend_size, box.lty=0)}
  dev.off()
...
DMM_cluster_nonPS_KW <- function (metabolomics_df, 
                                  metadata_df,
                                  variable_to_compare,
                                  max_cluster=5,
                                  uniqueID_col, 
  ...
  # Ensure the metabolomics data is in the correct orientation
  metabolomics_matrix <- as.matrix(metabolomics_df)
  if (ncol(metabolomics_matrix) < nrow(metabolomics_matrix)) {
    metabolomics_matrix <- t(metabolomics_matrix)
  }
  
  # Check if the metabolomics data contains absolute counts
  countData_check <- floor(metabolomics_matrix)
  if (all(countData_check == 0)) {
    stop("Please use metabolomics data with absolute counts. The current input contains relative abundance")
  }
  
  # Set random seed for reproducibility
  set.seed(1234)
  
  # Perform DMM clustering
  tse_dmn <- mia::runDMN(tse, name="DMN", k = 1:max_cluster)
  
  # Display the stats for each iteration
  getDMN(tse_dmn)
  
  plotDMNfit_output <- paste0("DMM_model_fit_graph_", output_name, ".pdf")
  plotDMNFit <- plotDMNFit(tse_dmn, type = "laplace")
  pdf(file = plotDMNfit_output, width = 16, height = 18)
  show(plotDMNFit)     
  dev.off()
  ...
  # Create a data frame for goodness of fit
  laplace <- sapply(1:length(getDMN(tse_dmn)), function(i) getDMN(tse_dmn)[[i]]@goodnessOfFit[[3]])
  laplace_df <- data.frame(Model_fit = laplace, `Number of Dirichlet components` = 1:length(laplace))
  
  # Group samples and return DMNGroup object that contains a summary
  dmn_group <- calculateDMNgroup(tse_dmn, variable = variable_to_compare, exprs_values = "counts", k = DMN_best_fit, seed = 1234)
  
  # Mixture weights (rough measure of the cluster size)
  DMM_comp_weight <- DirichletMultinomial::mixturewt(getBestDMNFit(tse_dmn))
  
  # Samples-cluster assignment 
  DMM_cluster_assign <- data.frame(Cluster_num = as.factor(DirichletMultinomial::mixture(getBestDMNFit(tse_dmn), assign = TRUE)))
  DMM_cluster_assign[[uniqueID_col]] <- row.names(DMM_cluster_assign) # Add the unique identifier to the DMM_cluster_assign dataframe
  
  assign(paste0("DMM_cluster_KW_", output_name), DMM_cluster_assign, envir = .GlobalEnv)
  
  # Write DMM csv file
  csv_output_DMM <- paste0("DMM_cluster_KW_", output_name, ".csv")
  write.csv(DMM_cluster_assign, csv_output_DMM)
  
  # Merge cluster assignment with metadata
  DMM_sample_var_interest <- metadata_df %>% dplyr::select(variable_to_compare)
  
  DMM_cluster_assign_v2 <- merge(DMM_cluster_assign, DMM_sample_var_interest, by.x = uniqueID_col, by.y = uniqueID_col)
  DMM_cluster_assign_v3 <- DMM_cluster_assign_v2 %>% dplyr::group_by(!!sym(variable_to_compare), Cluster_num) %>% tally()
  colnames(DMM_cluster_assign_v3) <- c(variable_to_compare, "Cluster_num", "n")
  
  DMM_cluster_assign_v4 <- DMM_cluster_assign_v3 %>%
    dplyr::group_by(!!sym(variable_to_compare)) %>%
    dplyr::mutate(perc = n / sum(n)) %>%
    dplyr::arrange(perc) %>%
    dplyr::mutate(labels = scales::percent(perc)) # Get percentage for the piechart
  ...
  if (!missing(DMM_cluster_color)){ # If DMM_cluster_color is not missing then replace the color 
    for(i in 1:length(DMM_cluster_color)) {
      cluster_color_t[i] <- DMM_cluster_color[i] # Replace the selected default color with selected 
    }
  } 
  
  graph_DMM_all <- list() # Make an empty list
  tt <- 1
  for(i in sort(unique(DMM_cluster_assign_v4[[variable_to_compare]]))){
    DMM_cluster_assign_v4 <- DMM_cluster_assign_v4 %>% dplyr::arrange(!!sym(variable_to_compare), Cluster_num)
    DMM_cluster_assign_v5 <- DMM_cluster_assign_v4[DMM_cluster_assign_v4[[variable_to_compare]] %in% i,]
    graph_DMM <- DMM_cluster_assign_v5 %>%
      ggplot(aes(x = "", y = n, fill = Cluster_num)) +
      geom_bar(stat = "identity", width = 1, color = "white") +
      geom_label(aes(label = labels, size = 14, fontface = "bold"), color = "white", 
                 position = position_stack(vjust = 0.5), 
  ...
  # Combine DMM plot will give you pie charts  
  combine_DMM_plot <- ggarrange(plotlist = graph_DMM_all, nrow = 1, common.legend = TRUE, legend = "bottom")
  combine_DMM_plot_output <- paste0("DMM_model_cluster_group_", output_name, ".pdf")
  pdf(file = combine_DMM_plot_output, width = 5 * (tt - 1), height = 5)
  show(combine_DMM_plot)              
  dev.off()
  ...
  # Set compare.test to either Fisher's exact test or chi-square test
  if (compare_stats == "chisquare"){
    compare.test <<- function(a, b) {
      return(chisq.test(cbind(a, b)))
    }
  ...
  # Different combinations of comparisons
  myComparisons <- as.list(as.data.frame(combn(unique(DMM_cluster_assign_v2[[variable_to_compare]]), 2))) 
  combine_DMM_plot_v2 <- ggplot(DMM_cluster_assign_v4, aes(x = !!sym(variable_to_compare), y = n)) +
    geom_bar(aes(fill = Cluster_num), stat = "identity", position = "dodge") +
    geom_signif(comparisons = myComparisons, test = "compare.test", map_signif_level = FALSE, step_increase = 0.1) +
    scale_fill_manual(values = cluster_color_t) +
    theme(axis.title.x = element_blank(),
          axis.title.y = element_blank(),
          axis.title = element_text(face = "bold", size = 18),
  ...
  combine_DMM_plot_output2 <- paste0("DMM_model_cluster_group_", output_name, "_V2.pdf")
  pdf(file = combine_DMM_plot_output2, width = 5 * (tt - 1), height = 5)
}
